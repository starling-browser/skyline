...
// Two windows, one GPU device, one render thread each. Each window draws and
// presents on its own thread, so neither window's vsync wait stalls the other.
//
...
using var gpu = WindowGpu.CreateContext(winA, surface: new WindowSurfaceOptions());
var surfaceB = WindowGpu.CreateSurface(gpu, winB);
...
/// are shared across both windows' render threads.
/// </summary>
